...
mod condcompile {
    ...
    async fn run_nts_pool_ke(nts_pool_ke_config: NtsPoolKeConfig) -> std::io::Result<()> {
        ...
        let certificate_chain_file =
            std::fs::File::open(&nts_pool_ke_config.certificate_chain_path).map_err(|e| {
                io_error(&format!(
...
pub async fn nts_pool_ke_main() -> Result<(), ()> {
    println!("Not enabled, enable `unstable_nts-pool` feature at build time to build functional pool ke server");
    Ok(())
}
