...
mod tests {
    ...
    fn test_remove_clock_with_link_is_forbidden() {
        let filter = LinkFilter::<StdKalmanStorage<()>>::empty(Timestamp::UNIX_EPOCH);
        let (filter, clock_1) = filter.add_clock((0.0, 0.0).into(), (0.0, 0.0).into(), 0.0).unwrap();
        let (filter, clock_2) = filter.add_external_clock().unwrap();
        let (filter, link_1) = filter.add_untracked_link(clock_1, clock_2).unwrap();

        assert_eq!(filter.clone().remove_clock(clock_1).unwrap_err(), AlgoError::ClockInUse(clock_1, link_1));
        assert_eq!(filter.clone().remove_external_clock(clock_2).unwrap_err(), AlgoError::ClockInUse(clock_2, link_1));
