...
Both ntpd, the ntp reference implementation, and ntpd-rs can serve a similar role on Unix systems. This guide is aimed to help those migrating machines currently running ntpd to ntpd-rs. We assume here experience with the [ntpd configuration format](https://docs.ntpsec.org/latest/ntp_conf.html#_reference_clock_support). A user with no or little `ntpd` experience may be better of following our [getting started guide](getting-started.md).
...
**Defaults:** `ntpd` and `ntpd-rs` may use different default values for some properties. When migrating, pay particular attention toml

* `???`/`minimum-agreeing-peers`
...
Note that unlike `ntpd`, ntpd-rs does not have a default ip address on which it listens for nts-ke traffic, and this needs to be explicitly provided. The port is optional however and defaults to the standard value 4460.
...
Note that in contrast to `ntpd`'s `cookie` option, here the full path needs to be specified, and there is no default path.
