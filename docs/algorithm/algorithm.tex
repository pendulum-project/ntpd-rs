...
\end{pmatrix}
\end{align}
a random vector when it's components $W_1$ and $W_2$ are random variables. For a random vector we define it's covariance matrix as the matrix of the covariances between its components. For the example vector W, it's covariance matrix is given by
\begin{align}
U &= \begin{pmatrix}
