...
async fn main() -> std::io::Result<()> {
    let listener = tokio::net::TcpListener::bind("0.0.0.0:4461").await?;
...
pub(crate) async fn key_exchange_server(
    ...
    let tls_cfg = {
        ...
        let certs = load_certs("test-keys/sample.pem").unwrap();
        // Load private key.
        let key = load_private_key("test-keys/sample.rsa").unwrap();
