...
pub(crate) async fn key_exchange_server(
    ...
    let certs = load_certs(Path::new("test-keys/example.com+5.pem")).unwrap();
    let mut keys = load_keys(Path::new("test-keys/example.com+5-key.pem")).unwrap();
    ...
    let mut result = BoundKeyExchangeServer::new(stream, config)?.await?;
    ...
    result
        .tls_connection
        .writer()
        .write_all(NTS_TIME_NL_RESPONSE)
        .unwrap();
