...
struct SystemTask<C: NtpClock, Controller: TimeSyncController<Clock = C>> {
    ...
    #[cfg(target_os = "linux")]
    ptp_networking_ipv4: Option<&'static statime_netptp::NetworkManager<Ipv4Addr>>,
    #[cfg(target_os = "linux")]
    ptp_networking_ipv6: Option<&'static statime_netptp::NetworkManager<Ipv6Addr>>,
    #[cfg(target_os = "linux")]
    csptp_manager:
...
impl<C: NtpClock + Sync, Controller: TimeSyncController<Clock = C>> SystemTask<C, Controller> {
            ...
            SourceCreateParameters::Csptp(ref params) => match params.addr {
                IpAddr::V4(addr) => {
                    let network = if let Some(network) = self.ptp_networking_ipv4 {
                        ...
                        let manager: &'static statime_netptp::NetworkManager<_> =
                            Box::leak(Box::new(manager));
                        self.ptp_networking_ipv4 = Some(manager);
                        manager
                    };
                    let controller = self
                        .controller
                ...
                IpAddr::V6(addr) => {
                    let network = if let Some(network) = self.ptp_networking_ipv6 {
                        network
                    } else {
                        let manager = match statime_netptp::NetworkManager::new() {
                            Ok(manager) => manager,
                        ...
                        let manager: &'static statime_netptp::NetworkManager<_> =
                            Box::leak(Box::new(manager));
                        self.ptp_networking_ipv6 = Some(manager);
                        manager
                    };
                    let controller = self
                        .controller
    ...
    #[cfg(target_os = "linux")]
    async fn add_csptp_server(&mut self, config: crate::daemon::config::CsptpServerConfig) {
        let network_v4 = if let Some(network) = self.ptp_networking_ipv4 { network } else {
            let manager = match statime_netptp::NetworkManager::new() {
                Ok(manager) => manager,
                Err(e) => {
        ...
        let network_v6 = if let Some(network) = self.ptp_networking_ipv6 { network } else {
            let manager = match statime_netptp::NetworkManager::new() {
                Ok(manager) => manager,
                Err(e) => {
