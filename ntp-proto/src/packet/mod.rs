...
pub use crypto::{AesSivCmac256, Cipher, CipherProvider, EncryptionResult};
...
impl<'a> NtpPacket<'a> {
    ...
    pub fn deserialize<CP: CipherProvider>(
        data: &'a [u8],
        cipher: &CP,
    ...
    pub fn serialize<CP: CipherProvider>(
        &self,
        w: &mut Cursor<&mut [u8]>,
        cipher: &CP,
    ) -> std::io::Result<()> {
...
mod tests {
    ...
    fn test_undersized_nonce() {
        ...
        //should not crash
        assert!(NtpPacket::deserialize(&input, &()).is_err());
    }
